...
struct node {
	...
	node(T value) : value(value), leftNode(nullptr), rightNode(nullptr) {
...
public :
	...
	bool findValue(T value, node<T>** outNode);
	void destroyTree();
	void deleteValue(T value);
	void print();

	template <typename U>
	friend std::ostream& operator<< (std::ostream& os, const BTree<U>& tree);
private:
	node<T>* root;

	void destroyTree(node<T>* subNode);
	void insertValue(T value, node<T>* subNode);
	node<T>* findValue(T value, node<T>* subNode);
	node<T>* deleteValue(T value, node<T>* subNode);
	node<T>* minNodeFromGivenNode(node<T>* subNode);
	void print(node<T>* subNode);
};
